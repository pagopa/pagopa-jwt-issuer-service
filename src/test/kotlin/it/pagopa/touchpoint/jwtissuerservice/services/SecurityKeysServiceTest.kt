...
class SecurityKeysServiceTest {
    ...
    private val azureSecretConfig : AzureSecretConfigProperties = AzureSecretConfigProperties(name = "testName", password = "testPassword")
    private val securityKeysService = SecurityKeysService(secretClient = secretClient, azureSecretConfig = azureSecretConfig)
    ...
    fun `Should get key store successfully`() = runTest {
        ...
        val secretTest = KeyVaultSecret("testName", generatePKCS12CertificateAsBase64(keyStore, azureSecretConfig.password))
        given { secretClient.getSecret(any()) }.willReturn(Mono.just(secretTest))

        val obtainedKeyStore = securityKeysService.getKeyStore().block()
        assertThat(keyStore.getKey("testAlias", azureSecretConfig.password.toCharArray()).encoded).isEqualTo(obtainedKeyStore?.getKey("testAlias", azureSecretConfig.password.toCharArray())?.encoded)
        assertThat(keyStore.getCertificate("testAlias").publicKey.encoded).isEqualTo(obtainedKeyStore?.getCertificate("testAlias")?.publicKey?.encoded)
        assertThat(keyStore.aliases().toList()).containsExactlyInAnyOrderElementsOf(obtainedKeyStore?.aliases()?.toList())
    }
